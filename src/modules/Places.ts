...
export const getPlaces = createAsync<void, PlaceType[]>(
  'SEARCH',
  async (_, __, ___, client) => {
    const response = await client.get(apiUrl)
    const currentPosition: Position = getCurrentPosition()

    const placesState: PlaceType[] = response.data.results.bindings.map(
      (result: any) => {
